...
#ifdef SCREEN_SPLIT
const int SPARSE_ROWS = 2*SCREEN_HEIGHT/5;
#else
...
// solid (dark) colour
//#define BACKGROUND_FILL(_x, _count, _colour) LCD_FILL_COLOUR(_count, _colour)
// alternating dots of colour
uint16_t background_ctr;
#define BACKGROUND_FILL(_count, _colour) { long ctr = _count; while (ctr--) if (background_ctr++ % 4) LCD_ONE_BLACK(); else LCD_FILL_COLOUR(1, _colour); }
